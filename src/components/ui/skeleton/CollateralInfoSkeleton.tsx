  ...
  return (
    ...
    >
      ...
      <div className="p-4 md:p-6 xl:p-8 md:pt-4 xl:pt-4">
        <div className="w-full flex h-auto items-center md:h-52 border rounded-lg  text-mist border-frost bg-glass">
          <div className="w-2/5 h-fit flex items-center justify-center">
            <div
              className="rounded-full w-24 h-24 md:w-28 md:h-28 lg:w-36 lg:h-36 bg-mist-light flex self-center"
          ...
          <div className="w-3/5">
            <div className="mt-4 flex space-x-4 font-bold text-lg ">
              <div
                className="w-16 h-4 md:w-20 md::h-6 rounded bg-mist-light"
            ...
            <div className="mt-6 md:mt-8 text-xs font-[400]">
              <div
                className="w-36 h-4 md:w-56 md:h-5 rounded bg-mist-light"
                data-testid="loading-liquidation-threshold"
              ></div>
              <div
                className="w-36 h-4 md:w-56 md:h-5 rounded bg-mist-light mt-2"
                data-testid="loading-liquidation-penalty"
              ></div>
              <div
                className="w-36 h-4 md:w-56 md:h-5 rounded bg-mist-light mt-5 md:mt-7"
                data-testid="loading-my-collateral"
              ></div>
              <div
                className="w-20 h-4 rounded bg-mist-light my-2"
        ...
        <div className="hidden md:grid w-full h-10 rounded mt-4 bg-[#ffffff05] px-8 grid-cols-4 text-white items-center">
          {collateralHeaderItems.map((item, index) => {
            return (
              <div
                key={index}
                className="text-mist text-xs font-semibold"
          ...
          {Array.from({ length: 3 }).map((_, rowIndex) => (
            ...
            >
              <div className="flex items-center gap-x-2">
                <div
                  className="rounded-full w-8 h-8 bg-mist-light flex self-center"
        ...
        <div className="md:hidden mt-4 border rounded-lg  text-mist border-frost bg-glass">
          {Array.from({ length: 3 }).map((_, rowIndex) => (
            ...
            >
              <div className="flex items-center gap-x-2">
                <div
                  className="rounded-full w-8 h-8 xl:w-10 xl:h-10 bg-mist-light flex self-center"
                  data-testid={`loading-asset-${rowIndex + 1}`}
                ></div>
                <div className="flex relative">
                  <div className="font-bold text-xs xl:text-sm">
                    <div
                      className="w-12 h-4 rounded bg-mist-light"
                      data-testid={`loading-asset-amount-${rowIndex + 1}`}
                    ></div>
                  </div>
                </div>
              ...
              <div className="flex justify-between mt-4">
                ...
                <div className="flex flex-col items-end text-2xs">
                  ...
                  ></div>
                  <div
                    className="w-10 h-4 mt-1 rounded bg-mist-light"
                    data-testid={`loading-asset-amount-${rowIndex + 1}`}
                  ></div>
                </div>
              </div>
