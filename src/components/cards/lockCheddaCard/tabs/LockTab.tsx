...
export const LockTab: FC<LockTabProps> = ({
  ...
  return (
    <div data-testid="lock-card-content" className="mt-4 lg:mt-6">
      ...
      <div className="text-mist text-sm mt-2">{subTitle}</div>
      {!isCheddaLocked ? (
        <>
          ...
          />
          <div className="mt-6 text-2xs lg:text-xs text-mist">
            Select Lock Period
          </div>
          ...
          <div className="flex space-between gap-x-2 mt-5">
            {LOCKTIMES.map((item, index) => (
              <button
                ...
                className={`bg-[#0e327059] rounded-lg flex justify-center items-center w-full relative lock-time-box transition-all ${lockTime === item.value ? "modal-button" : ""}`}
              >
          ...
          {showWarning ? (
            <div className="text-error mt-2 text-2xs lg:text-xs">
              Select lock period before proceeding.
            </div>
          ) : (
            <div className="text-warning mt-2 text-2xs lg:text-xs">
        ...
        <div className="mt-6 text-5xl text-white relative">
          ...
          <div className="text-3xs md:text-2xs lg:text-xs text-mist flex gap-x-1 mt-6 justify-between items-center">
            <div>Extend or add more assets to your locked assets</div>
            <button
              onClick={openManageLockModal}
              className="bg-[#00000030] px-2 lg:px-3 font-bold py-3 text-3xs lg:text-sm rounded  hover:opacity-80 relative"
            >
